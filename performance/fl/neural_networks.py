...
class ResNetRS(nn.Module):
    ...
    def __call__(self, x, train=True):
        x = STEM(
            bn_momentum=self.bn_momentum, bn_epsilon=self.bn_epsilon, scale=self.scale, name="STEM_1"
        )(x, train)
...
class ResNetRS50(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        x = ResNetRS(
            ...
            [
                {
                    "input_filters": round(64 * self.pw),
                    "num_repeats": 3
                },
                {
                    "input_filters": round(128 * self.pw),
                    "num_repeats": 4
                },
                {
                    "input_filters": round(256 * self.pw),
                    "num_repeats": round(6 * self.pd)
                },
                {
                    "input_filters": round(512 * self.pw),
                    "num_repeats": 3
                },
            ],
...
#     # variables = model.init(jax.random.PRNGKey(42), np.zeros((1, 32, 32, 3)))

#     # RESNETRS50 START
#     model = ResNetRS50(10)
#     with open("ResNetRS50.variables", 'rb') as f:
#         variables = serialization.msgpack_restore(f.read())
#     init_variables = model.init(jax.random.PRNGKey(42), np.zeros((1, 32, 32, 3)))
